...
int a[100];
void ShotTAMA(TEKI* teki,TAMA *tama);
//======================================
// オープニング
//======================================
void Opening(void) {
...
void MainRoutine(void) {
	...
	for (int i = 0; i < TAMA_MAX; i += 1) {
		...
		if (tama[i].x < 0 || tama[i].x>1440 || tama[i].y < 0 || tama[i].y>900) {
			tama[i].flag = 0;
		}
		//Zキーが押されてtriggerが１０以上でtamaが存在しなければ発射
		if (KeyFlag['Z'] && jiki.trigger > 1 && tama[i].flag == 0) {
			tama[i].x = jiki.x;
			tama[i].y = jiki.y;
			jiki.trigger = 0;
			tama[i].flag = 1;
	...
	for (int i = 0; i < ETAMA_MAX; i++) {
		...
		if (etama[i].x < 0 || etama[i].x>1440 || etama[i].y < 0 || etama[i].y>900) {
			etama[i].flag = 0;
		}
	}
	...
	for (int t = 0; t < TEKI_MAX; t += 1) {
		...
		for (int i = 0; i < ETAMA_MAX; i++) {
			if (teki[t].trigger > 20) {
				ShotTAMA(&teki[t], &etama[i]);
			}
		}
	}
