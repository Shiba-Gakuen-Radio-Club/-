...
void Action(void) {
	if (count == 0) {
		...
		jiki.speed=8;
		goal.x = 1400;
		for (int i = 0; i < 9; i++)block[i].flag = 1, block[i].x = i*100, block[i].y = 810 - 100;
		block[9].x = 80;
		block[9].y = 400;
		block[9].flag = 1;
		for(int i=10;i<B_MAX;i++)block[i].flag = 1, block[i].x = i * 100, block[i].y = 810 - 100;
	}
	...
	//	if (jiki.y + 200 < block[i].y && jiki.x + 100 >= block[i].x && jiki.x + 100 <= block[i].x + 50 && block[i].y + 100 >= jiki.y)jiki.x -= jiki.x + 100 - block[i].x;
		if (jiki.y + 200 >= block[i].y && jiki.x + 100 > block[i].x && jiki.x < block[i].x + 100&&block[i].y+100>=jiki.y) G = 0, jiki.y -= jiki.y+200-block[i].y;
		//if (jiki.y + 200 < block[i].y && jiki.x  < block[i].x+100 && jiki.x  > block[i].x + 50 && block[i].y + 100 > jiki.y)jiki.x += block[i].x+100-jiki.x;
		//else	if (block[i].y >= jiki.y && jiki.x + 100 > block[i].x && jiki.x < block[i].x + 100 && block[i].y  <= jiki.y)  jiki.y += block[i].y+100-jiki.y;
		if (jiki.x > 1000)
			block[i].x -= jiki.scrole;
		if (jiki.x < 200)
			block[i].x -= jiki.scrole;
	...
	SpriteBlt(hDC_Back, goal.x, 300, 100, 100, h_block, 0, 0, blockmask);
	if (jiki.x > 1000)goal.x -= jiki.scrole;
	if (jiki.x < 200)goal.x -= jiki.scrole ;

	if (jiki.y > 900)GameFlag = ENDING;
	if (jiki.x > goal.x)GameFlag = ENDING;
	jiki.scrole = 0;

	if (KeyFlag[VK_RIGHT]) 	jiki.x += jiki.speed,jiki.scrole+=jiki.speed,anime+=1, jiki.mode = 1;
	if (KeyFlag[VK_LEFT]) 	jiki.x -= jiki.speed, jiki.scrole -= jiki.speed, anime-=1,jiki.mode = 0;
	if (KeyFlag[VK_UP] && G == 0) G -= 60, jiki.land = 0;
	if (jiki.x > 1200)jiki.x = 1200;
	if (jiki.x < 100)jiki.x = 100;
	G++, jiki.y += G / 7;
	//if (jiki.land==1)G=0;
}
//==============================================================================================
